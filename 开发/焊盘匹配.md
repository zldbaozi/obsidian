...
void EditWidget::on_toolButton_similarPads_clicked()
{
    ...
    // templateDCodes: 用于存储选中的 D码 (std::string)，快速匹配常规焊盘
    std::unordered_set<std::string> templateDCodes;
    // preTemplatesGeom: 存储异形焊盘或无D码焊盘，需进行深度几何校验
    ...
    for (QGraphicsItem* item : selectedItems)
    {
        ...
        if (pItem && pItem->getGerberInfo()) {
            ...
            std::string dcode = info->GetDcode();

            // 严格区分“异形焊盘”。
            // 只要宽高接近0（Region/Polygon），就强制走几何匹配，不允许直接用D码匹配。
            // 这样可以防止 所有异形焊盘因为D码相同(例如都是D10) 而被混淆。
            bool isZeroSize = (qAbs(info->GetWidth()) < 0.0001 && qAbs(info->GetHeight()) < 0.0001);

            if (!dcode.empty() && !isZeroSize) {
    ...
    QString logMsg = QString("Templates: DCode(Normal)=%0, Geometry(Complex)=%1, Extracted(Post)=%2")
                         .arg(templateDCodes.size()).arg(preTemplatesGeom.count()).arg(postTemplates.count());
    ...
    for (QGraphicsItem* item : allItems)
    {
        ...
        if (target && target->getGerberInfo()) {
            ...
            bool tIsZeroSize = (qAbs(tInfo->GetWidth()) < 0.0001 && qAbs(tInfo->GetHeight()) < 0.0001);

            // 策略A: D码极速匹配
            if (!tIsZeroSize && !tDcode.empty() && templateDCodes.count(tDcode)) {
                isMatch = true;
            }
            // 策略B: 几何特征匹配 (兜底)
